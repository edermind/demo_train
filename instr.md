...
| HQ-CLI (VLAN 200, ≥16) | `192.168.0.32/27` | `192.168.0.33` | DHCP |
...
### 2.2. Зона безопасности (обязательно для маршрутизации!)
```
security-zone trusted
exit
```

### 2.3. Интерфейс к ISP + маршрут по умолчанию
...
### 2.4. VLAN + router-on-a-stick (один порт, субинтерфейсы)
```
...
interface tunnel gre 1
  ...
  exit
```
> Анонсируй сети HQ (VLAN-субинтерфейсы) в OSPF.
